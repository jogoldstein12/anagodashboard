...
import { UniStatsRow } from "./UniStatsRow";
import { TradeHistory } from "./TradeHistory";
import { SignalCard } from "./SignalCard";

export function UniDashboardClient() {
  const status = useQuery(api.uni.getUniStatus);
  const trades = useQuery(api.uni.getUniTrades, { limit: 20 });
      ...
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-white flex items-center gap-3">
            <span>🪸</span> Uni Dashboard
          </h1>
          <p className="text-white/50 mt-1">Kalshi CPI prediction market trader</p>
        </div>
